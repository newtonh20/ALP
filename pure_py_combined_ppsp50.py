...
"""
__all__ = ["partition_at_level", "modularity", "generate_dendrogram", "generate_dendogram", "induced_graph"]
...
def alp_farthest_ecc(g, subgraphs, opt = False, trials=10, dijkstra_sample_nodes=100):
    ...
    for g_p in subgraphs.values():
        ...
        map(lambda s: threading.Thread(target=run_dij, args=(s,)).start(), samples)
...
heuristics = enum(DIJKSTRA=1, ALT=2, ALP=3, PCD=4, ALT_OPT=5, ALP_OPT=6,ALT_ND_PATHMAX=11,ALP_ND_PATHMAX=12)
...
alt_landmark_froms = {}
def ALT(s, t):
    global g, alp_landmarks, cluster_nodes, num_alt_landmarks, landmarks,num_alt_calculations, num_alt_estimates, tid,gid
    ...
    if not landmarks:
        ...
        if em == embedding_methods.RANDOM:
            ...
            landmarks = random_ls_opt(g, num_alt_landmarks, .00001, 10)
        ...
        prep_sql = "INSERT INTO preprocessing VALUES (NULL," + str(tid) + "," + str(heuristics.ALT_ND_PATHMAX) + "," + str(gid) + "," + str(prep_time) + ")"
        '''if DEBUG:
    ...
    return max(list(map(lambda l: abs(alt_landmark_froms[l][s] - alt_landmark_dists[l][t]), landmarks)))
...
def get_landmark_paths(l, rgraph):
    ...
    alp_landmark_dists[l] = single_source_dijkstra_path_length(rgraph, l, target_cutoffs=alp_landmarks[:]+sub[:], only_targets=True)
...
def ALP(v, end):
    global g, alp_landmarks, cluster_nodes, node_clusters, num_alp_calculations, num_alp_estimates, tid, gid, em
    ...
    if not alp_landmarks:
        ...
        if em == embedding_methods.RANDOM:
            ...
            alp_landmarks = alp_random_ls_opt(g, cluster_nodes, .0001, 10)
        ...
        if len(alp_landmarks) > 2:       
            max_t = min(int(len(alp_landmarks)/8)+8,15)
            from joblib import Parallel, delayed
            results = Parallel(n_jobs=max_t,verbose=50)(delayed(get_landmark_paths)(l, reverse_graph) for l in alp_landmarks)
            for result in results:
                alp_landmark_dict.update(result[0])
                alp_landmark_dists[result[1][0]] = result[1][1]
                alp_landmark_froms.update(result[2])
            '''t_count = 0
        ...
        prep_sql = "INSERT INTO preprocessing VALUES (NULL," + str(tid) + "," + str(heuristics.ALP_ND_PATHMAX) + "," + str(gid) + "," + str(prep_time) + ")"
        query_insert(prep_sql)
    ...
    try:
        ...
        estimates = [0, abs(l1_v-l2_l1)-t_l2, abs(l1_v-t_l2)-l2_l1, abs(l2_l1-t_l2)-l1_v]
        ...
        if l2_l1 != 0:
            estimates.extend([(abs(l1_v - l2_l1) * abs(l2_l1 - t_l2) - l1_v * t_l2) / l2_l1])
        num_alp_calculations += 6
        ...
        if alp_landmark_dists[l_2][l_1] > alp_landmark_dists[l_1][v] + t_l2:
            ...
            estimates.extend([abs(l1_v-l2_l1)+abs(t_l2-l2_l1)-l2_l1])
        else:
            num_alp_calculations += 2
        return max(estimates)
    except KeyError:
        #no reachable path. return shortest edge weight
        num_alp_estimates += 1
        return min([g[v][u].get('weight', 1) for u in g[v]])

last_num_alp_landmarks = 0
def test(n=1, debug=False, eid=0, tid=0, gid=0,st_list=[], max_labels=0):
    global cluster_nodes, node_clusters, g,num_alp_calculations,num_alp_estimates,em,landmarks,num_alt_calculations, num_alt_estimates,em, num_alt_landmarks, last_num_alp_landmarks
    ...
    if max_labels > 0:
        for k in reversed(range(1, int(math.sqrt(len_nodes)))):
    ...
    node_clusters = nclusters
    #ALP will have this many landmarks as well
    print "Cluster Nodes:", str(num_landmarks)
    lets_alp = True
    if last_num_alp_landmarks == num_landmarks:
        lets_alp = False
    else:
        last_num_alp_landmarks = num_landmarks
    ...
    for i in st_list:
        ...
        try:
            ...
            #ALP Query and Error
            if em != embedding_methods.FARTHEST_D and lets_alp:
                num_alp_calculations = 0
                num_alp_estimates = 0
                start = time.time()
            ...
            if first:
                ...
                alt_sql = "(NULL," + str(tid) + "," + str(heuristics.ALT_ND_PATHMAX) + "," + str(em) + "," + str(s) + "," + str(t) + "," + str(len(path)) + "," + str(len(landmarks)) + "," + str(alt_time) + "," + str(alt_size) + "," + str(num_alt_calculations) + "," + str(num_alt_estimates) + "," +str(alt_path_weight) + ")"
                #alp_sql = "(NULL," + str(tid) + "," + str(heuristics.ALP_ND_PATHMAX) + "," + str(em) + "," + str(s) + "," + str(t) + "," + str(len(path)) + "," + str(num_landmarks) + "," + str(alp_time) + "," + str(alp_size) + "," + str(num_alp_calculations) + "," + str(num_alp_estimates) + ")"
                #ALP Query and Error
                if em != embedding_methods.FARTHEST_D and lets_alp:
                    alp_sql = "(NULL," + str(tid) + "," + str(heuristics.ALP_ND_PATHMAX) + "," + str(em) + "," + str(s) + "," + str(t) + "," + str(len(path)) + "," + str(num_landmarks) + "," + str(alp_time) + "," + str(alp_size) + "," + str(num_alp_calculations) + "," + str(num_alp_estimates) + "," +str(alp_path_weight) + ")"
                #Dijkstra Query
...
if __name__ == "__main__":
    experiment_id = query_insert("INSERT INTO experiments (description, start_time) values ('ALT vs ALP PathMax (4-points) with Fixed Memory and Farthest Selection', '" + time.strftime('%Y-%m-%d %H:%M:%S') + "')")
    if experiment_id:
        first = True
        num_n = 1000
                    ...
                    for gr in gs:
                        ...
                        try:
                            ...
                            if graph_id:
                                ...
                                if first:
                                    sizes = [25000000, 50000000, 100000000]
                                    first = False
                                else:
                                        ...
                                        if o in [2,3,9,10,12]:
                                            ...
                                            if trial_id:
                                                ...
                                                if eid and tid and gid:
                                                    print "Setting max_labels to", str(ml)
                                                    test(num_n, st_list=st_l, eid=experiment_id, tid=trial_id, gid=graph_id, max_labels=ml)
                                                else:
