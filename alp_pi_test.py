...
heuristics = enum(DIJKSTRA=1, ALT=2, ALP=3, PCD=4, ALT_OPT=5, ALP_OPT=6,ALT_ND_PATHMAX=11,ALP_ND_PATHMAX=12, ALT_ND_PATHMAX_KMEANS=13,ALP_ND_PATHMAX_KMEANS=14, ALT_ND_PATHMAX_EDGE_BETWEENNESS=15, ALP_ND_PATHMAX_EDGE_BETWEENNESS=16, ALT_ND_PATHMAX_LV_EB=17, ALP_ND_PATHMAX_LV_EB=18, ALT_PI_LV=19, ALP_PI_LV=20)
...
def get_graph_features(h):
    ...
    d = runf(nx.estrada_index) if b < 1000 and not h.is_directed() else -1
    ...
    q = runf(nx.average_shortest_path_length) if c < 3000 else -1
    try:
        r = nx.connected_double_edge_swap(g, nx.number_of_edges(g)) if c < 5000 else -1
    except:
        r = -1
...
def ALT(s, t):
    ...
    if not landmarks:
        ...
        prep_sql = "INSERT INTO preprocessing VALUES (NULL," + str(tid) + "," + str(heuristics.ALT_PI_LV) + "," + str(gid) + "," + str(prep_time) + ")"
        if DEBUG:
...
def ALP(v, end):
    ...
    if not alp_landmarks:
        ...
        prep_sql = "INSERT INTO preprocessing VALUES (NULL," + str(tid) + "," + str(heuristics.ALP_PI_LV) + "," + str(gid) + "," + str(prep_time) + ")"
        query_insert(prep_sql)
...
def test(n=1, k=1, debug=False, eid=0, tid=0, gid=0,st_list=[], max_labels=0):
    global cluster_nodes, node_clusters, g,num_alp_calculations,num_alp_estimates,em,landmarks,num_alt_calculations, num_alt_estimates,em, num_alt_landmarks, last_num_alp_landmarks, alp_landmarks, graph_name
    ...
    def alp_query_graph(i):
        ...
        try:
            ...
            if first:
                ...
                first = False
                #record system resource consumption
                stats_file = open(graph_name + '_alp_' + str(len(alp_landmarks)) + '.stats', 'w')
                stats_file.write(str(psutil.virtual_memory()))
                stats_file.write(str(psutil.cpu_times()))
                stats_file.write("\n")
                stats_file.close()
            else:
                alp_sql = "(NULL," + str(tid) + "," + str(heuristics.ALP_PI_LV) + "," + str(em) + "," + str(s) + "," + str(t) + "," + str(len(path)) + "," + str(num_landmarks) + "," + str(alp_time) + "," + str(alp_size) + "," + str(num_alp_calculations) + "," + str(num_alp_estimates) + "," +str(alp_path_weight) + ")"
                #ALP Query and Error
    ...
    def alt_query_graph(i):
        ...
        try:
            ...
            if first:
                ...
                first = False
                #record system resource consumption
                stats_file = open(graph_name + '_alt_' + str(len(landmarks)) + '.stats', 'w')
                stats_file.write(str(psutil.virtual_memory()))
                stats_file.write(str(psutil.cpu_times()))
                stats_file.write("\n")
            else:
                alt_sql = "(NULL," + str(tid) + "," + str(heuristics.ALT_PI_LV) + "," + str(em) + "," + str(s) + "," + str(t) + "," + str(len(path)) + "," + str(len(landmarks)) + "," + str(alt_time) + "," + str(alt_size) + "," + str(num_alt_calculations) + "," + str(num_alt_estimates) + "," +str(alt_path_weight) + ")"
    ...
    stats_file = open(graph_name + '_dijkstra.stats', 'w')
    stats_file.write(str(psutil.virtual_memory()))
    stats_file.write(str(psutil.cpu_times()))
    stats_file.write("\n")
    stats_file.close()
    for st in st_list:
        dijkstra_query_graph(st)
...
if __name__ == "__main__":
    ...
    if experiment_id:
        ...
        for i in range(10):
            ...
            g_name = "NULL"
            data_path = "/home/pi/dissertation_data/large"
