...
def ALP(v, end):
    ...
    if not alp_landmarks:
        ...
        pstart = time.time()
        alp_landmarks = alp_random_ls_opt(g, cluster_nodes, node_clusters)
        ...
        for l in alp_landmarks:
            ...
            print "Target cutoffs for", str(l), ":", cluster_nodes
...
if result or test:
    experiment_id = result[0][0] if not test else 0
    num_n = 1000
    for i in range(10):
        ...
        for j in range(10):
            ...
            print "Running trial", str(j), "for n=", str(num_n)
            graph_type = choice(range(12))
            g_name = "NULL"
            ...
            if graph_type == 0:
                ...
                g = nx.cycle_graph(num_n)
                g_name = 'NETWORKX.CYCLE_GRAPH'
            elif graph_type == 4:
                g = nx.ladder_graph(num_n)
            ...
            if result or test:
                graph_id = result[0][0] if not test else 0
                #print graph_insert
                print "Running trial on", g_name, "graph."
                sys.stdout.flush()
                if True:
                    ...
                    for k in range(5):
                        ...
                        if result or test:
                            trial_id = result[0][0] if not test else 0
                            print "Trial ID:", str(trial_id)
                            #choose different landmarks
                            sys.stdout.flush()
                            test(1000, nx.number_of_nodes(g), debug=False, eid=experiment_id, tid=trial_id, gid=graph_id)
                            ALP_landmark_dict = dict()
                            landmarks = []
                            alp_landmarks = []
                            num_alt_calculations = 0
