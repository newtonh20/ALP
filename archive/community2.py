    ...
    for node in graph :
        ...
        for neighbor, datas in graph[node].iteritems() :
            ...
            if partition[neighbor] == com :
                if neighbor == node :
                    inc[com] = inc.get(com, 0.) + int(weight)
                else :
...
def generate_dendogram(graph, part_init = None) :
    ...
    #current_graph = nx.Graph(graph.subgraph(graph.nodes()))
    ...
    def init(self, graph, part = None) :
        ...
        if part == None :
            for node in graph.nodes() :
                ...
                self.loops[node] = int(graph.get_edge_data(node, node,
            ...
            for node in graph.nodes() :
                ...
                deg = int(graph.degree(node, weigh = 'weight'))
...
def __remove(node, com, weight, status) :
    ...
    status.internals[com] = int( status.internals.get(com, 0.) -
